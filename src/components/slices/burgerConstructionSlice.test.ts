...
import { IBurgerConstructor, addIngredient, burgerConstructorSlice, changeIngredient, deleteIngredient, initialState } from './burgerConstructionSlice';
...
const MAIN_INGREDIENT_ID = '2';
const SAUCE_INGREDIENT_ID = '3';
...
describe('Тесты удаления ингридиентов', () => {
    it('тест удаления котлеты', () => {
        const testState = burgerConstructorSlice.reducer(burgerState, deleteIngredient(MAIN_INGREDIENT_ID));
    ...
    it('тест удаления соуса', () => {
        const testState = burgerConstructorSlice.reducer(burgerState, deleteIngredient(SAUCE_INGREDIENT_ID));
        expect(testState.ingredients).toHaveLength(1);
